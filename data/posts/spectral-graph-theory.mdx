...
**Theorem (Existence of $R(3, 3)$):** For a graph $G = (V, E)$, there exists a constant $R(3, 3)$ such that whenever $|V| \ge R(3, 3)$, either $G$ contains $K_3$ or $\overline{G}$ contains $K_3$, that is, either $\operatorname{Tr}(A^3) > 0$ or $\operatorname{Tr}(A^3) > 0$.
