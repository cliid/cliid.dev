...
export default function Tweet({
  ...
  const quoteTweet =
    referenced_tweets && referenced_tweets.find((t: { type: string }) => t.type === 'quoted');

  return (
    <div className="tweet tw-rounded-lg tw-border tw-border-gray-200 dark:tw-border-gray-800 tw-px-6 tw-py-4 tw-my-4 tw-w-full tw-bg-white dark:tw-bg-gray-900">
      <div className="tw-flex tw-items-center">
        <a
          className="tw-flex tw-h-12 tw-w-12"
          href={authorUrl}
          target="_blank"
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="author tw-flex tw-flex-col tw-ml-4 tw-no-underline"
        >
          <span
            className="tw-flex tw-items-center tw-font-bold tw-text-gray-900 dark:tw-text-gray-100 tw-leading-5"
            title={author.name}
          >
            {author.name}
            {author.verified ? (
              <svg
                ...
                className="tw-ml-1 tw-text-blue-500 dark:tw-text-white tw-inline tw-h-4 tw-w-4"
                viewBox="0 0 24 24"
              >
                <g fill="currentColor">
          ...
          <span className="tw-text-gray-500" title={`@${author.username}`}>
            @{author.username}
          </span>
        </a>
        <a className="tw-ml-auto" href={authorUrl} target="_blank" rel="noopener noreferrer">
      ...
      <div className="tw-mt-4 tw-mb-1 tw-leading-normal tw-whitespace-pre-wrap tw-text-lg tw-text-gray-700 dark:tw-text-gray-300">
        {formattedText}
      </div>
      {media && media.length ? (
        <div
          className={
            media.length === 1
              ? 'tw-inline-grid tw-grid-cols-1 tw-gap-x-2 tw-gap-y-2 tw-my-2'
      ...
      <div className="tw-flex tw-text-gray-700 dark:tw-text-gray-300 tw-mt-2">
        <a
          ...
          rel="noopener noreferrer"
        >
          <svg className="tw-mr-2" width="24" height="24" viewBox="0 0 24 24">
            <path
