...
function NavItem({
  ...
  return (
    <Link href={href} className="tw-flex tw-items-center tw-justify-center">
      <div
        aria-label={typeof content === 'string' ? content : ''}
        className="tw-flex tw-items-center tw-justify-center tw-h-10 tw-font-semibold tw-text-gray-600 tw-text-md dark:tw-text-gray-400 hover:tw-text-black dark:hover:tw-text-white"
      >
        {content}
      </div>
    </Link>
...
export default function Template({
  ...
  const meta = {
    ...
    description: description ?? `Jiwu Jang`,
    type: type ?? 'website',
    image,
    date
  };
        ...
        >
          ...
          >
            ...
            <div className="tw-grid tw-grid-flow-col tw-gap-x-4">
              ...
              <div className="tw-hidden sm:tw-block">
                <NavItem currentPath={router.asPath} href="/guestbook" content="Guestbook" />
              </div>
            </div>
          </div>
        </nav>
        <main className="tw-flex tw-flex-col tw-w-full tw-px-4 sm:tw-px-8">
          <div
            className="tw-flex tw-flex-col tw-justify-center tw-w-full tw-mx-auto"
            style={{
              maxWidth
            }}
          >
            <h1 className="tw-mb-4 tw-text-4xl tw-font-extrabold tw-text-black dark:tw-text-white">
              {heading ?? title ?? ''}
            </h1>
