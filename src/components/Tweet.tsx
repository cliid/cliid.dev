...
export default function Tweet({
  ...
  return (
    <div className="tw-w-full tw-px-6 tw-py-4 tw-my-4 tw-border tw-rounded-lg tweet">
      <div className="tw-flex tw-items-center">
        <Link className="tw-flex tw-w-12 tw-h-12" href={authorUrl}>
          <NextImage
            alt={author.username}
            height={48}
            width={48}
        ...
        <Link href={authorUrl} className="tw-flex tw-flex-col tw-ml-4 tw-no-underline author">
          <span
            className="tw-flex tw-items-center tw-font-bold tw-leading-5 tw-text-gray-900 dark:tw-text-gray-100"
            title={author.name}
          >
            {author.name}
            {author.verified ? (
              <svg
                ...
                className="tw-inline tw-w-4 tw-h-4 tw-ml-1 tw-text-blue-500 dark:tw-text-white"
                viewBox="0 0 24 24"
              >
                <g fill="currentColor">
        ...
        <Link className="tw-ml-auto" href={authorUrl}>
      ...
      <Link className="tw-text-sm tw-text-gray-500 hover:tw-underline" href={tweetUrl}>
        <time title={`Time Posted: ${createdAt.toUTCString()}`} dateTime={createdAt.toISOString()}>
          {format(createdAt, 'h:mm a - MMM d, y')}
        </time>
      </Link>
      <div className="tw-flex tw-mt-2 tw-text-gray-700 dark:tw-text-gray-300">
        <Link
